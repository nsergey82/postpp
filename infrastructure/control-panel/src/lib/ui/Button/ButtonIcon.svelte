<script lang="ts">
	...
	interface IButtonProps extends HTMLButtonAttributes {
		...
		bgColor?: 'black' | 'white' | 'gray' | 'primary' | 'secondary' | 'danger' | string;
		iconSize?: 'sm' | 'md' | 'lg' | number | string;
		iconColor?: 'black' | 'white' | 'gray' | 'primary' | 'secondary' | 'danger' | string;
		strokeWidth?: number;
	}
...
>
	{#if isLoading || isSubmitting}
		<div class="loading loading-spinner loading-lg absolute {cn(classes.iconColor)}"></div>
	{:else}
